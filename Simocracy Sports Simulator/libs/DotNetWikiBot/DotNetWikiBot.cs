// DotNetWikiBot Framework 3.14 - designed to make robots for MediaWiki-powered wiki sites
...
// Copyright (c) Iaroslav Vassiliev (2006-2015) codedriller@gmail.com
...
namespace DotNetWikiBot
{
	...
	public class Site
	{
		...
		private void LogIn()
		{
			if (!useApi) {
				...
				string respStr = PostDataAndGetResult(apiPath +
		...
		/// <summary>Gets and parses results of specified custom API query.</summary>
	...
	public class Page
	{
		...
		/// <summary>Removes the namespace prefix from <see cref="Page.title"/>.</summary>
			...
			string newParameterValue, bool firstTemplateOnly)
		{
			...
			foreach (string template in GetTemplates(true, false)) {
				if (templateTitleRegex.IsMatch(template)) {
					...
					newTemplate = site.FormatTemplate(templateTitle, parameters, template);
	...
	public class PageList
	{
		...
		/// Not all queries are supported. The function does not
		...
		/// before calling GetApiQueryResult.</param>
		/// <param name="limit">Maximum number of resultant strings to fetch.</param>
		/// <example><code>
		...
		/// <summary>Compares pages by titles.</summary>
	...
	public class Bot
	{
		...
		public static readonly Version version = new Version("3.14");
		...
		static Bot()
		{
			...
			Console.Write(botVer + " " + version + "\n" +
				"Copyright (c) Iaroslav Vassiliev, 2006-2015, GNU General Public License 2.0\n\n");
		...
		/// <summary>Gets localized (translated) version of the specified bot's
